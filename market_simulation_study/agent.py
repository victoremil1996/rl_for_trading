...
class MuPolicyNetwork(nn.Module):
...
class ActorCriticAgent:
    def __init__(self,
        ...
        self.all_trades = np.array([[0, 0]])
        self.buy_volume = n_volume
        self.sell_volume = n_volume
        self.spread = None
        self.mid_price = None
        self.buy_order = pd.DataFrame(np.array([[self.buy_price, self.buy_volume, self.latency, self.agent_id]]),
                                      columns=["buy_price", "buy_volume", "latency", "agent_id"],
    ...
    def get_state_features(self, state: dict, n_returns=5):
        ...
        features.append(np.sum(np.abs(state["volume"]))/2)
    ...
    def update(self, state: dict, exploration_mode=False):
        ...
        pos_penalty = (self.position_penalty * position) * (self.position_penalty * position)
        reward = torch.tensor([new_pnl - pnl - pos_penalty])
